...
const LiveObject: React.FC<{ obj: SceneObject, isSelected: boolean, onSelect: () => void, isPlaying: boolean }> = ({ obj, isSelected, onSelect, isPlaying }) => {
  ...
  useFrame((state) => {
    if (isPlaying && meshRef.current && obj.behaviors?.includes('spin')) {
      meshRef.current.rotation.y += 0.01;
    }
    if (isPlaying && meshRef.current && obj.behaviors?.includes('float')) {
          ...
          <div className="flex-1 flex items-center space-x-1 h-full">
            {['layout', 'shading', 'world', 'data', 'rendering', 'pipelines'].map((ws) => (
              <button
                ...
                className={`px-8 h-full text-[9px] font-black uppercase transition-all flex items-center border-b-2 tracking-widest ${
                  activeWorkspace === ws ? 'text-cyan-400 border-cyan-500 bg-cyan-600/5' : 'text-slate-600 hover:text-cyan-400 border-transparent'
                }`}
              >
                {ws}
              </button>
